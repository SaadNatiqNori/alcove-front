      ...
      {groups.map((group, gi) => (
        ...
        >
          <div className="mx-auto max-w-[1440px] border-t border-[#1C1F2A] px-[16px] tablet:px-[38px] pt-[40px] tablet:pt-[50px] pb-0 md:px-[38px] md:pt-[70px]">
            <div className="grid grid-cols-1 gap-[55px] tablet:gap-[68px] md:grid-cols-[minmax(0,0.9fr)_minmax(0,2.55fr)] md:gap-16">
              {/* Title column */}
              <div data-feat-item>
                <h2
                  className="m-0 whitespace-pre-line text-[42px] tablet:text-[52px] md:text-[50px] font-[420] leading-[1] tracking-[-0.04em] text-[#1C1F2A]"
                  style={{
                ...
                {group.subtitle && (
                  <p className="m-0 mt-5 text-[15px] tablet:text-[18px] leading-[1.5] text-[#5A6472]">
                    {group.subtitle}
                  </p>
                )}
              </div>
              ...
              <div>
                {group.rows.map((row, ri) => (
                  <div
                    key={row.feature ?? ri}
                    data-feat-item
                    className="grid grid-cols-1 justify-between gap-2 border-b border-[#1C1F2A] pt-[28px] pb-[28px] tablet:pt-[36px] tablet:pb-[36px] first:pt-0 md:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)] md:gap-12 md:pt-[46px] md:pb-[46px] md:first:pt-0"
                  >
                    <h3
                      className="m-0 self-center text-[24px] tablet:text-[32px] md:text-[38px] font-normal leading-[1.15] tracking-[-0.02em] text-[#1C1F2A]"
                      style={{ textBoxTrim: 'trim-both', textBoxEdge: 'cap alphabetic' }}
                    >
                      {row.feature}
                    </h3>
                    <p
                      className="m-0 w-full self-center text-[13px] tablet:text-[18px] leading-[1.4] tracking-[0] text-[#5A6472] tablet:leading-[1.25] md:ml-auto md:max-w-[312px] md:text-[18px] md:leading-[1.2] pt-[6px] md:pt-0"
                      style={{ textBoxTrim: 'trim-both', textBoxEdge: 'cap alphabetic' }}
                    >
